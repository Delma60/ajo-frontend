...
function PaymentDueBanner({
  ...
  // find user in member
  const currentMember = groups
    .flatMap((g) => g.members)
    .find((m) => m.id === currentUserId);
  const unpaid = groups.filter(
    (g) => g.status === "active" && !currentMember?.hasPaid,
  );
    ...
    <div className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-amber-700 mt-0.5">
          {unpaid.map((g) => g.name).join(", ")} — make your payment to stay in
          good standing.
        </p>
      </div>
      <Button
        size="sm"
        variant="secondary"
        className="rounded-xl shrink-0 bg-amber-100 text-amber-800 hover:bg-amber-200"
      >
        Pay now
      </Button>
    </div>
...
function CircleRow({
  ...
  const isDueSoon =
    ...
    group?.status === "active" &&
    (dueIn === "Today" || dueIn === "Tomorrow" || dueIn === "Overdue");
    ...
    <Link href={`groups/${group.id}`}>
      <div className="flex items-center gap-4 px-5 py-4 hover:bg-zinc-50 transition-colors group cursor-pointer">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 text-[11px] text-zinc-400 flex-wrap">
            ...
            {currentMember?.myTurn && (
              <span>
                Turn {currentMember.myTurn} of {group.max_members}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="text-right shrink-0 hidden sm:block">
          <p
            className={`text-[12px] font-semibold mb-0.5 ${isDueSoon ? "text-amber-600" : "text-zinc-700"}`}
          >
...
function SummaryStats({ groups = [] }: { groups?: IGroup[] }) {
  ...
  const totalContributed = active.reduce(
    (s, g) => s + Number(g.contribution),
    0,
  );
  const nextPayoutGroup = active.sort(
    (a, b) =>
      new Date(a.nextPayout).getTime() - new Date(b.nextPayout).getTime(),
  )[0];
  ...
  const stats = [
    ...
    {
      label: "Next payout",
      value: nextPayoutGroup
        ? new Date(nextPayoutGroup.nextPayout).toLocaleDateString("en-NG", {
            day: "numeric",
            month: "short",
          })
        : "--",
    ...
    {
      label: "Total saved",
      value: formatNaira(groups.reduce((s, g) => s + g.saved, 0)),
      icon: TrendingUp,
    },
  ];
...
export default function MyCirclesPage() {
  ...
  const filtered = user?.groups
    ? user?.groups?.filter((g) => {
  ...
  return (
    <div className="min-h-full bg-zinc-50/40">
      <div className=" px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-0.5">
              {user?.groups?.length} circle
              {user?.groups?.length !== 1 ? "s" : ""} · {activeGroups?.length}{" "}
              active
            </p>
          </div>
        ...
        {/* Stats */}
        <SummaryStats groups={user?.groups} />
        ...
        {filtered?.length === 0 ? (
          ...
          <Card
            variant="default"
            className="overflow-hidden divide-y divide-zinc-100"
          >
            {filtered?.map((group) => (
              <CircleRow
                key={group.id}
